...
|---|---|---|---|
| 1 | 只定义了 `EventTypeId` 字段，没列事件目录 | **§5.4 给出完整事件目录**：三类共 47 个核心事件类型，每个写明命名空间/名称/payload 字段/语义/生产者/消费者/示例 | 用户指出："不了解的人看完完全不知道都有哪些 event，然后分别有什么用" |
...
public interface WorldEvent {
    ...
    /** 类型标识。**不是 enum** —— 见 §5.4 的目录与 §5.4.5 的注册机制。 */
    EventTypeId typeId();
...
| 因素 | 取值 | 说明 |
|---|---|---|
| `urgency` | 0–1 | 危险 0.95、疼痛 0.9、来电 0.8、消息通知 0.5、环境光变化 0.2 |
| `salience` | 0–1 | 刺激本身的突出程度（音量 × 关系权重 × 是否刚看过手机） |
| `occurredAt` | Instant | 同优先级时先到先处理 |

**现实中的实时事件清单**：

| 事件 | urgency | salience 来源 | 说明 |
|---|---|---|---|
| `device.phone.sound-emitted.v1` | 0.5 | 音量 × 是否聊天软件 | 手机响了 |
| `device.phone.vibrated.v1` | 0.45 | 震动强度 × 手机位置 | 手机震了（在包里会弱很多） |
| `body.thermal-threshold-breached.v1` | 0.6 | 越界程度 | 觉得冷 / 觉得热 |
| `body.pain-threshold-breached.v1` | 0.9 | 疼痛值 | 疼 |
| `body.hunger-threshold-breached.v1` | 0.5 | 饥饿值 | 饿了 |
| `environment.odor-detected.v1` | 0.4 | 浓度 | 闻到味道 |
| `environment.strong-light.v1` | 0.3 | 照度 | 强光刺眼 |
...
> （`CoreEventCatalog`，§5.4.5），**不是 Java enum** —— 新增事件不需要改任何 `switch`、
...
- **类别**：`A` = 持续影响（→ Ledger）、`B` = 实时感官（→ Queue）、`C` = 计划（→ PlanBoard）；
  多字母表示同时属于多类。
...
|---|---|---|---|---|
| `environment.temperature-changed.v1` | A | `TemperatureChanged(oldCelsius, newCelsius, windSpeed, humidity)` | 她所处地点的气温变了。**本身不是"她冷"** —— 它只是一条事实，影响由 Body 的稳态模型算出 | `EnvironmentRefreshJob` → `BodyHomeostasisHandler` |
| `environment.humidity-changed.v1` | A | `HumidityChanged(oldRatio, newRatio)` | 湿度变了。影响体感温度与 `wetness` 增速 | `EnvironmentRefreshJob` → `BodyHomeostasisHandler` |
| `environment.wind-changed.v1` | A | `WindChanged(oldMps, newMps)` | 风速变了。风速显著放大热量散失（风寒效应） | `EnvironmentRefreshJob` → `BodyHomeostasisHandler` |
| `environment.precipitation-started.v1` | A, B | `PrecipitationStarted(kind, mmPerHour)` | 开始下雨/下雪。**同时是实时事件** —— 她会立刻注意到（看到/听到），而不仅仅是"慢慢变湿" | `EnvironmentRefreshJob` → `BodyHomeostasisHandler`（A） / `AuditoryChannel`+`VisualChannel`（B） |
| `environment.precipitation-stopped.v1` | A | `PrecipitationStopped(kind, durationMinutes)` | 雨停了。移除 `WetnessEffect` 的持续输入 | `EnvironmentRefreshJob` → `BodyHomeostasisHandler` |
| `environment.illuminance-changed.v1` | A, B | `IlluminanceChanged(oldLux, newLux, phase)` | 光照变了。**跨阈值时是实时事件**（"天黑了该开灯了"） | `EnvironmentRefreshJob` → `VisualChannel` |
| `environment.weather-changed.v1` | A | `WeatherChanged(oldCondition, newCondition)` | 天气分类变化（晴→雨）。用于她的计划约束（`WeatherConstraint`） | `EnvironmentRefreshJob` → `PlanConstraintEvaluator` |
| `environment.odor-detected.v1` | B | `OdorDetected(kind, concentration, durationSec)` | 闻到气味（饭香、烟味、臭味） | `EnvironmentRefreshJob` → `OlfactoryChannel` |
| `environment.refresh-failed.v1` | — | `EnvironmentRefreshFailed(reason, providerId)` | 拿不到真实环境数据。**这是一条可观测事件，不是异常** —— 降级要留痕 | `EnvironmentRefreshJob` → `DiagnosticsHandler` |

#### 5.4.2 设备域（`device.*`）—— DigitalDeviceWorld

| 事件类型 | 类别 | payload | 语义 | 生产者 → 消费者 |
|---|---|---|---|---|
| `device.phone.notification-raised.v1` | B | `PhoneNotificationRaised(phoneId, applicationId, reason, channel)` | 手机把一个通知摆到了她面前。**不含任何消息正文** | `Phone.NotificationSystem` → `AuditoryChannel`/`TactileChannel`/`VisualChannel` |
| `device.phone.sound-emitted.v1` | B, A | `PhoneSoundEmitted(phoneId, channel, soundProfile, volume, durationMs)` | 手机真的发出了声音（音量 > 0 才有这条事件） | `Phone.AudioSystem` → `AuditoryChannel` |
| `device.phone.vibrated.v1` | B | `PhoneVibrated(phoneId, pattern, strength)` | 手机震动了 | `Phone.VibrationSystem` → `TactileChannel` |
| `device.phone.screen-woke.v1` | B | `PhoneScreenWoke(phoneId, reason, illuminanceLux)` | 屏幕亮起（暗环境下是一条显著的视觉事件） | `Phone.ScreenSystem` → `VisualChannel` |
| `device.phone.battery-low.v1` | B | `PhoneBatteryLow(phoneId, level, estimatedMinutesLeft)` | 电量低。**这会改变她的计划** —— 她可能得先充电 | `Phone.PowerState` → `Mind`（经 Attention） |
| `device.phone.battery-depleted.v1` | A | `PhoneBatteryDepleted(phoneId)` | 手机没电了。**她从此收不到通知**，这是"她失联"的可解释原因 | `Phone.PowerState` → `Ledger`（移除所有该设备的影响） |
| `device.phone.state-changed.v1` | A | `PhoneStateChanged(phoneId, field, oldValue, newValue)` | 手机状态变了（静音开关、位置、网络） | `Phone` → `Ledger` |
| `device.alarm.ringing.v1` | B, C | `AlarmRinging(deviceId, alarmId, label)` | 闹钟响了 | `ClockDevice` → `AuditoryChannel` + `PlanBoard` |
| `device.calldial.ringing.v1` | B | `IncomingCallRinging(deviceId, callerAccountId, channel)` | 来电。**urgency 很高（0.8）** | `Phone` → `AuditoryChannel` |
| `device.generic.state-changed.v1` | A | `DeviceStateChanged(deviceId, field, oldValue, newValue)` | 任意设备的通用状态变化（供第三方设备使用） | 任意 `Device` → `Ledger` |

#### 5.4.3 身体域（`body.*`）—— Human 内部产生的世界事件

> 注意：这些事件**由 Body 产生**，经 EventFabric 进入 Mind。
> 它们看起来像"内部事件"，但走的是同一条边界 —— 因为 Body 属于 Human，
> Mind 要感知 Body 的状态，也必须经过一条**显式的通道**。
> 这条设计让"她没意识到自己饿了"成为一个合法状态。

| 事件类型 | 类别 | payload | 语义 | 生产者 → 消费者 |
|---|---|---|---|---|
| `body.thermal-threshold-breached.v1` | B | `ThermalThresholdBreached(sensation, warmth, threshold, delta)` | 保暖值跌破（或升破）舒适阈值。`sensation ∈ {COLD, HOT}` | `HomeostasisModel` → `TactileChannel` → `Mind` |
| `body.hunger-threshold-breached.v1` | B | `HungerThresholdBreached(hunger, threshold)` | 饿了 | `HomeostasisModel` → `Mind` |
| `body.thirst-threshold-breached.v1` | B | `ThirstThresholdBreached(thirst, threshold)` | 渴了 | `HomeostasisModel` → `Mind` |
| `body.fatigue-threshold-breached.v1` | B | `FatigueThresholdBreached(fatigue, threshold, activityDemand)` | 累了 | `HomeostasisModel` → `Mind` |
| `body.sleep-pressure-threshold-breached.v1` | B | `SleepPressureThresholdBreached(pressure, threshold)` | 困了 | `HomeostasisModel` → `Mind` |
| `body.pain-threshold-breached.v1` | B | `PainThresholdBreached(pain, threshold, source)` | 疼 | `HomeostasisModel` → `Mind` |
| `body.physiology-drift.v1` | — | `PhysiologyDrift(field, oldValue, newValue, ratePerHour)` | 生理量的**有意义的**漂移（不是每 tick 都发）。用于界面图表与审计 | `HomeostasisModel` → `DiagnosticsHandler` |
| `body.clothing-changed.v1` | A | `ClothingChanged(added, removed, newInsulation)` | 穿着变了（穿上/脱下）。**这是持续影响** —— 改变 heatLoss 系数 | `Body` → `Ledger` |
| `body.sensation-started.v1` | A | `SensationStarted(kind, intensity, bodyPart)` | 持续体感开始（如"手一直很凉"） | `Body` → `Ledger` |

#### 5.4.4 对象 / 计划 / 系统域

| 事件类型 | 类别 | payload | 语义 | 生产者 → 消费者 |
|---|---|---|---|---|
| `object.wearable.equipped.v1` | A | `WearableEquipped(objectId, personId, insulation)` | 穿上一件衣物。**产生持续影响**（§3.2.5） | `Wardrobe` → `Ledger` |
| `object.wearable.removed.v1` | A | `WearableRemoved(objectId, personId)` | 脱下。移除对应持续影响 | `Body` → `Ledger` |
| `object.placed.v1` | A | `ObjectPlaced(objectId, fromPlace, toPlace)` | 东西被放到了别处（如"手机放在客厅"→ 她听不见） | 任意 `WorldObject` → `Ledger` |
| `plan.item-triggered.v1` | C | `PlanItemTriggered(revisionId, planItemId, intentType, window)` | 计划项到点了，该开始了 | `PlanScheduler` → `Life` |
| `plan.item-completed.v1` | C | `PlanItemCompleted(revisionId, planItemId, actualDuration)` | 计划项做完了 | `Life` → `PlanBoard` |
| `plan.item-interrupted.v1` | C | `PlanItemInterrupted(revisionId, planItemId, causeEventId)` | 计划项被打断（**这只是一条记录，重排是另一件事**） | `Life` → `PlanBoard` |
| `plan.revision-created.v1` | C | `PlanRevisionCreated(revisionId, previousRevisionId, reason, mutations[])` | 计划表产生了新版本。**这是重排的落点** | `PlanReplanner` → `PlanBoard` |
| `plan.validation-failed.v1` | — | `PlanValidationFailed(reason, rejectedIntent)` | LLM 提出的重排没通过校验。**可观测，不静默丢弃**（§3.5.6） | `PlanValidator` → `DiagnosticsHandler` |
| `world.object-created.v1` | — | `WorldObjectCreated(objectId, typeId, displayName)` | 世界里新增了一个对象 | `World` → `DiagnosticsHandler` |
| `world.object-destroyed.v1` | A | `WorldObjectDestroyed(objectId, typeId)` | 对象没了（手机坏了）。**移除它的全部持续影响** | `World` → `Ledger` |
| `simulation.clock-ticked.v1` | — | `ClockTicked(simulationTime, realTime, speedFactor)` | 仿真时钟走了一格。**这是驱动一切的节拍**，但 deliberately 不进入任何认知队列 | `SimulationClock` → 各 tick 消费者 |
| `system.degraded.v1` | — | `SystemDegraded(component, reason, fallbackMode)` | 某个子系统降级了（环境 API 挂了、设备没配对）。**必须可见** | 各组件 → `DiagnosticsHandler` |

#### 5.4.5 事件目录的实现：`CoreEventCatalog`
 ...
 * {@code EventHandlerRegistry}（§5.4.6）。
 */
public final class CoreEventCatalog {

    private static final Map<EventTypeId, EventTypeDefinition> CORE = Map.ofEntries(
        entry(def("environment", "temperature-changed", 1)
                .category(STATE_EFFECT)
                .payloadClass(TemperatureChanged.class)
                .description("她所处地点的气温发生了变化。本身不是'她觉得冷' —— "
                           + "那是一个需要经过衣物保暖与稳态模型计算之后才可能产生的事实。")
                .producer("EnvironmentRefreshJob")
                .consumer("BodyHomeostasisHandler")
                .build()),
        // ... 其余 46 条
    );

    /** 运行时可查：这个类型存在吗？它的 payload 是什么？谁产生、谁消费？ */
    public Optional<EventTypeDefinition> describe(EventTypeId id);

    /** 全部已知类型 —— 界面用它渲染"事件目录"页。 */
    public Collection<EventTypeDefinition> all();

    /** 按命名空间分组 —— 界面用它做分类展示。 */
    public Map<String, List<EventTypeDefinition>> byNamespace();
}
```

#### 5.4.6 事件路由：`EventHandlerRegistry`（不是 switch）
