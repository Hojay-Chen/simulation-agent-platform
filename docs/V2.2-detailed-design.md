...
WorldRuntime.pulse()
  ...
  └─ ④ 对每个 ACTIVE 的 human: humanActor.pulse(now)
         ...
         ├ b. body.drainSensoryStimuli()   B 类刺激出队
         ├ c. mind.absorbAll(stimuli, attentionContext)
         │       —— 注意闸门: 不是每一条刺激都进得了她的意识
         ├ d. 若 Mind 产出 Decision → actionFabric.dispatch(command)
...
"她不看"。注意积压的是 B 类队列；A 类的账本**仍然每 tick 结算**
（外面的温度不会因为她被暂停就不冷了），只是没有人去看结算结果。
恢复时 `RecoveryRuntime`（§8.5.6）要读的是**世界历史**，不是那堆积压的刺激。
...
它进 B 类队列，走 ④ 的 b→c 两步（注意：是**下一个** tick 的 ④，因为 ① 在 ④ 之前），
由 Mind 的注意闸门决定要不要理会。
