...
| 11 | 没有说 agent 的**身份、归属与生命周期**存在哪 —— 而 §8.4 的包结构里也没有任何一个包接得住旧 `persona` / `state` / `person` 包 | **§3.6 新章**：`human`（仿真身份）与 `agent_ownership`（平台归属 + 生命周期）拆两张表；`AgentLifecycle` 保留为枚举并写明判据；`Persona` 从存储降级为投影；§3.6.7 给出身份侧完整迁移表 | 这是一次包级审计发现的缺口：`server` 与 `openapi` 仍引用旧包 24 处，按原样删旧包**两个服务编译不过** —— 设计看似完整、迁移路径断在中间 |
...
#### 3.6.4 生命周期是一个**可以**用枚举的地方

`AgentLifecycle` 保留为枚举：

```java
public enum AgentLifecycle {
    ACTIVE,      // 认知循环在跑
    PAUSED,      // 认知循环停, 世界继续走
    ARCHIVED,    // 只读: 可以看她的历史, 不能再产生新事件
    DELETED;     // 软删: 数据在, 所有读写都拒
}
```
...
- `ACTIVE / PAUSED / ARCHIVED / DELETED` 是**平台自己定义的四个档位**，
  外部世界不会明天冒出一个第五种，也不需要第三方来扩展它 —— **所以它可以是枚举**；
- 对照组：`Activity.type`（"写作业/上班/运动…"）是**开放**的，
  第三方要能加"做实验" —— **所以它必须是多态实现类**。

**同一个文档里两种做法并存不矛盾，因为判据是同一条。**

#### 3.6.5 `PAUSED` 的语义：停的是认知，不是世界
...
| `persona.AgentLifecycle` | `human.lifecycle` 字段 + 同名枚举 | 保留枚举，改挂载点 |
...
|---|---|---|---|
| `human` | `id` | — | 仿真身份（已存在） |
| `agent_ownership` | `id` | `human_id` 唯一 | 归属 + 生命周期（新增） |
...
│   ├── AgentRegistry.java           （§3.6.7 —— 取代旧 persona.CompanionService）
│   └── AgentProfileProjector.java   （§3.6.6 —— 读模型, 现场投影, 不落库）
│
└── registry
